...
- Removed all `href="#"` / `works#` dead links from Works cards. Non-detail Showcase and Graphic Design items now render as disabled non-link cards instead of anchors, preventing broken hash navigation while keeping real detail links clickable. Verified with `pnpm run build`.
